...
| `GET /api/hidden` | Titles marked "not interested": `{ hidden: [{ id, title, at }] }` |
| `POST /api/hidden/:anilistId` | Hide a title from Discover, the schedule and the Homepage widget. Body `{ "title": "…" }` is optional and only labels the list. Search and the detail panel still show it |
| `DELETE /api/hidden/:anilistId` | Show it again |
